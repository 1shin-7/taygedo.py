"""Tower of Fantasy in-game monthly sign-in."""
...
from taygedo.cli.ht._helpers import HT_GAME_ID, resolve_ht_role_id
...
async def sign(
    ...
    async with client:
        ...
        try:
            sign_env = await client.nte_sign.sign_role(role_id=rid, game_id=HT_GAME_ID)
        except ApiError as exc:
            raise click.ClickException(f"签到失败: {exc.body}") from exc
        new_state_env = await client.nte_sign.get_state(game_id=HT_GAME_ID)
        new_state = new_state_env.data or state
        rewards_env = await client.nte_sign.get_rewards(game_id=HT_GAME_ID, role_id=rid)
        flush_session(client, account)
